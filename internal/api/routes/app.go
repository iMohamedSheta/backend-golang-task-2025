...
func RegisterRoutes() *gin.Engine {
	...
	api.POST("/login", authHandler.Login)                      // Done
	api.POST("/refresh-token", authHandler.RefreshAccessToken) // Done

	// Create User (register customer)
	userHandler := handlers.NewUserHandler()
	api.POST("/users", userHandler.CreateUser) // Done

	// Product view
	productHandler := handlers.NewProductHandler()
	api.GET("/products", productHandler.ListProducts)                 // Done
	api.GET("/products/:id", productHandler.GetProduct)               // Done
	...
	api.Use(middleware.Auth())
	{
		...
		// User Management
		api.GET("/users/:id", userHandler.GetUser)    // Done
		api.PUT("/users/:id", userHandler.UpdateUser) // Done

		// Product Management
		api.POST("/products", middleware.AdminOnly(), productHandler.CreateProduct)    // Done
		api.PUT("/products/:id", middleware.AdminOnly(), productHandler.UpdateProduct) // Done

		// Order Management (VIP)
		orderHandler := handlers.NewOrderHandler()
		api.POST("/orders", orderHandler.CreateOrder) // Working on it
